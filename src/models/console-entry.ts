...
export const ConsoleLevels = ['log', 'debug', 'info', 'warn', 'error'] as const
...
export function isValidTransition(from: ConsoleEntryState, to: ConsoleEntryState): boolean {
...
export function toBuffered(entry: StatefulConsoleEntry): StatefulConsoleEntry {
  if (entry.state !== ConsoleEntryState.Created) {
    throw new Error(`Invalid transition: ${entry.state} -> ${ConsoleEntryState.Buffered}`)
  }
...
export function toStreamed(entry: StatefulConsoleEntry): StatefulConsoleEntry {
  if (entry.state !== ConsoleEntryState.Buffered) {
    throw new Error(`Invalid transition: ${entry.state} -> ${ConsoleEntryState.Streamed}`)
  }
...
export function parseConsoleEntry(value: unknown): ConsoleEntry | undefined {
  const res = ConsoleEntrySchema.safeParse(value)
